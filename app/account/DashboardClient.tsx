                ...
                <div className="flex items-center gap-5 text-center md:text-left">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-white font-serif">
                      Loving your invitations?
                    </h3>
                    <p className="text-white/80 text-sm">
                      Share your experience with us and help other couples create their dream
                      wedding!
                    </p>
                  </div>
                </div>
              ...
              <CardContent className="flex flex-col items-center gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-slate-500 max-w-sm">
                    You haven't created any wedding invitations yet. Start building your dream
                    invite today!
                  </p>
                </div>
            ...
            {invitations.map((invite, i) => (
              ...
              >
                <Card className="overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border-slate-200 group">
                  <div
                    className={`h-36 bg-linear-to-r ${TIER_GRADIENT[invite.tier] || TIER_GRADIENT.basic} relative`}
                  >
                    {/* View count badge */}
                    ...
                    <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm rounded-xl p-3 shadow-sm border border-white/50">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span
                          className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${TIER_BADGE_STYLE[invite.tier] || TIER_BADGE_STYLE.basic}`}
                        >
                          {invite.tier} Plan
                        </span>
                  ...
                  <CardContent className="p-5 space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      <Link href={`/invite/${invite.slug}`} target="_blank" className="w-full">
                        <Button
                          variant="outline"
                          className="w-full justify-between group/btn border-slate-200"
                        >
                          View Invitation
                      ...
                      <Button
                        variant="ghost"
                        className="w-full justify-between text-slate-400 hover:text-slate-600"
                        onClick={() =>
                          navigator
                            .share?.({
              ...
              {isLoadingRsvps ? (
                ...
                <div className="py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    They will appear here once guests respond.
                  </p>
                </div>
              ) : (
                <div className="grid gap-2">
                  {rsvps.map((rsvp) => (
                    <div
                      key={rsvp.id}
                      className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl hover:border-rose-100 transition-colors shadow-xs group"
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${rsvp.attending ? 'bg-green-50 text-green-600' : 'bg-slate-50 text-slate-400'}`}
                        >
                          {rsvp.attending ? (
        ...
        <DialogContent className="max-w-lg p-0 border-0 rounded-3xl overflow-hidden shadow-2xl">
          <div className="bg-linear-to-r from-rose-50 to-amber-50 p-6 border-b border-rose-100 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1">
              We'd love to hear how we helped with your special day.
            </p>
          </div>
          <div className="p-4">
            <TestimonialForm
              initialEmail={email}
              initialName={
                invitations[0]?.brideName && invitations[0]?.groomName
                  ? `${invitations[0].brideName} & ${invitations[0].groomName}`
                  : ''
              }
              onSuccess={() => {
