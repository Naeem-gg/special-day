...
const TYPES = [
  ...
  },
  {
    ...
    color: '#EF4444',
    bg: '#FFF5F5',
    desc: 'Report an issue with our service',
  },
  {
    value: 'grievance',
    label: 'Grievance',
        ...
        {open && (
          ...
          >
            <div className="px-5 pb-5 border-t border-gray-50">
              <p className="text-sm text-gray-700 leading-relaxed mt-4 p-4 bg-gray-50 rounded-xl">
                {item.message}
              </p>
              ...
              {item.replies.length > 0 && (
                <div className="mt-4 space-y-3">
                  ...
                  {[...item.replies].reverse().map((reply) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${reply.isAdmin ? 'bg-rose-100 text-[#F43F8F]' : 'bg-gray-100 text-gray-500'}`}
                      >
                        {reply.isAdmin ? 'DN' : reply.authorName[0]}
                      </div>
                      <div
                        className={`max-w-[80%] px-4 py-3 rounded-2xl text-sm ${reply.isAdmin ? 'bg-linear-to-br from-[#F43F8F] to-[#c73272] text-white rounded-tr-sm' : 'bg-gray-100 text-gray-700 rounded-tl-sm'}`}
                      >
                        {reply.isAdmin && (
                          <p className="text-[10px] font-bold uppercase tracking-wider mb-1 text-white/70">
                            DNvites Support
                          </p>
                        )}
                        {reply.message}
...
export default function FeedbackClient({ initialThreads }: { initialThreads: FeedbackItem[] }) {
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(160deg, #FFF8F0 0%, #FEF0F6 60%, #FFF5E6 100%)',
    ...
    >
      ...
      <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-xl border-b border-rose-50 px-6 h-16 flex items-center justify-between">
        ...
        <h1 className="text-sm font-bold text-gray-700 uppercase tracking-widest">
          Help & Feedback
        </h1>
        <Link
          href="/"
          className="text-xs font-bold text-gray-400 hover:text-[#F43F8F] transition-colors uppercase tracking-wider"
        >
          ← Home
        </Link>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-5 py-16 space-y-16">
        ...
        >
          ...
          <p className="font-sans text-gray-500 max-w-lg mx-auto leading-relaxed">
            Share your feedback, report a complaint, raise a grievance, or suggest a feature. We
            read every single message and respond publicly.
          </p>
        </motion.div>
            ...
            {submitted ? (
              ...
              >
                ...
                <div className="p-6 border-b border-rose-50">
                  <p className="font-sans text-xs font-bold uppercase tracking-widest text-gray-400 mb-4">
                    What is this about?
                  </p>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            ...
                            style={{ color: active ? t.color : '#888' }}
                          >
                            {t.label}
                          </span>
                        </button>
                ...
                <div className="p-6 space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      <label className="font-sans text-xs font-bold uppercase tracking-wider text-gray-500 mb-1.5 block">
                        Your Name *
                      </label>
                      <input
                        required
                  ...
                  <button
                    ...
                    style={{
                      background: 'linear-gradient(90deg, #F43F8F, #c73272)',
                      height: '52px',
                    }}
                  >
                    {loading ? (
                  ...
                  <p className="font-sans text-[11px] text-center text-gray-400">
                    Your message will be posted publicly (without your email) so our team can
        ...
        >
          ...
          {loadingThreads ? (
            ...
            <div className="space-y-3">
              {threads.map((item) => (
                <FeedbackThread key={item.id} item={item} />
              ))}
            </div>
          )}
        </motion.div>
