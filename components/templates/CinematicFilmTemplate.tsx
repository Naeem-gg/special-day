...
function FilmStrip({ side }: { side: 'left' | 'right' }) {
  return (
    <div
      className={`absolute top-0 bottom-0 ${side === 'left' ? 'left-0' : 'right-0'} w-8 flex flex-col justify-around pointer-events-none`}
  ...
  return (
    <>
      ...
      >
          ...
          >
            ...
            <div className="text-center z-10">
              <motion.div
                ...
                className="film-flicker text-[12rem] font-light leading-none"
                style={{ color: '#C0A060', fontFamily: 'Georgia, serif' }}
              >
                {countdown}
              </motion.div>
              <div className="text-xs uppercase tracking-[0.5em] mt-4" style={{ color: '#666' }}>
                An Invitation Feature Presentation
              </div>
            </div>
          </motion.div>
        )}
        ...
        {/* ── HERO ─────────────────────────────────── */}
        <section
          className={`relative flex flex-col items-center justify-center text-center overflow-hidden px-10 ${isThumbnail ? 'min-h-[812px]' : 'min-h-screen'}`}
          style={{ background: '#0A0A0A' }}
        >
          <FilmStrip side="left" />
          <FilmStrip side="right" />
          ...
          {/* Scratch lines */}
          <div
            className="film-scratch absolute top-0 bottom-0 w-px left-1/3 z-20"
          ...
          {(showContent || isThumbnail) && (
            <div className="relative z-10 max-w-4xl film-flicker">
              ...
              <motion.p
                ...
                className="font-sans text-xs uppercase tracking-[0.6em] mb-6"
                style={{ color: '#888' }}
              >
              ...
              <motion.h1
                ...
                style={{ color: '#F0EAD6', fontSize: 'clamp(3rem, 10vw, 7rem)' }}
              >
                {brideName}
              </motion.h1>
              <motion.p
                ...
                animate={{ opacity: 1 }}
                transition={{ delay: 1 }}
                className="text-xl font-light italic my-3"
                style={{ color: '#C0A060' }}
              >
                and
              </motion.p>
              ...
              >
                <p className="font-sans text-sm tracking-widest" style={{ color: '#C0A060' }}>
                  {fmt}
                </p>
        ...
        <section className="py-24 px-4" style={{ background: '#F0EAD6' }}>
          <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              <h2 className="text-4xl font-light" style={{ color: '#1A1008' }}>
                The Schedule
              </h2>
              <div
            ...
            <div className="space-y-6">
              {events.map((ev, i) => (
                <motion.div
                  ...
                  style={{ background: 'white', borderColor: '#C0A060' }}
                >
                  <div className="flex items-start gap-4">
                    <div className="font-sans text-3xl font-light" style={{ color: '#C0A060' }}>
                      {String(i + 1).padStart(2, '0')}
                    </div>
                    <div>
                      <h3 className="text-2xl font-light mb-2" style={{ color: '#1A1008' }}>
                        {ev.name}
                      </h3>
                      {ev.description && (
                        <p className="font-sans text-sm mb-4" style={{ color: '#5A4A30' }}>
                          {ev.description}
                        </p>
                      )}
                      <div
        ...
        {gallery.length > 0 && (
          <section className="py-24 px-4" style={{ background: '#1A1008' }}>
            <div className="max-w-5xl mx-auto">
              ...
              >
                <h2 className="text-4xl font-light" style={{ color: '#F0EAD6' }}>
                  The Reel
                </h2>
              </motion.div>
              <div className="columns-2 md:columns-3 gap-4 space-y-4">
                {gallery.map((img, i) => (
                  <motion.div
                    ...
                    className="overflow-hidden rounded-none border border-amber-900/30"
                    style={{ breakInside: 'avoid' }}
                  >
                    <img
                      src={img.url}
                      alt=""
                      className="w-full object-cover transition-transform duration-700 hover:scale-105 sepia-[0.4] brightness-90"
                    />
                  </motion.div>
                ))}
              </div>
        ...
        <section className="py-20 text-center" style={{ background: '#0A0A0A' }}>
          ...
          >
            ...
            <h2 className="text-3xl font-light mb-2" style={{ color: '#F0EAD6' }}>
              The Finale — Join Us
            </h2>
