  ...
  return (
    <>
      ...
      <div className={isThumbnail ? "min-h-full" : "min-h-screen"} style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
        ...
        <section className={`relative flex flex-col items-center justify-center text-center overflow-hidden px-6 ${isThumbnail ? "min-h-[812px]" : inline ? "min-h-[700px]" : "min-h-screen"}`}
          ...
          <div className="relative z-10 max-w-3xl curtain px-4">
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}
              className="font-sans text-[9px] md:text-[10px] uppercase tracking-[0.6em] mb-4 md:mb-6" style={{ color: "#D4AF37" }}>
              بسم اللہ الرحمن الرحیم · With God&apos;s Blessings
            </motion.p>

            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}
              className="font-sans text-[10px] md:text-xs tracking-[0.4em] mb-6 md:mb-8" style={{ color: "#A08080" }}>
              The families of
            </motion.p>
            ...
            <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 1.3 }}
              ...
              style={{ fontSize: inline ? "clamp(2.5rem, 8vw, 4rem)" : "clamp(3rem, 10vw, 7.5rem)" }}>
              {brideName}
            </motion.h1>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }}
              className="text-lg md:text-xl font-light italic my-1 md:my-2" style={{ color: "rgba(212,175,55,0.6)" }}>
              &amp;
            </motion.p>
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}
              className="font-sans text-[11px] md:text-sm tracking-[0.3em] mt-6 md:mt-8 px-2" style={{ color: "#A08080" }}>
            ...
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 3 }}
              className="font-sans text-sm md:text-base tracking-widest mt-4 md:mt-6" style={{ color: "#D4AF37" }}>{fmt}</motion.p>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3.5 }}
              className="font-sans text-xs md:text-sm mt-2 px-4" style={{ color: "#806060" }}>{venue}</motion.p>
          </div>
        ...
        {/* ── COUNTDOWN ───────────────────────────── */}
        <section className="py-16 md:py-20 text-center px-4 relative overflow-hidden" style={{ background: "linear-gradient(180deg, #FFFBF0, #FFF5E0)" }}>
          {/* Decorative pattern */}
          <div className="absolute inset-0 pointer-events-none opacity-[0.03]"
            style={{ backgroundImage: "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23D4AF37' fill-opacity='1'%3E%3Cpath d='M30 0L30 60M0 30L60 30' stroke='%23D4AF37' stroke-width='0.5'/%3E%3C/g%3E%3C/svg%3E\")" }} />
          
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <p className="font-sans text-[10px] md:text-xs uppercase tracking-[0.5em] mb-3" style={{ color: "#D4AF37" }}>✦ Save The Date ✦</p>
            <h2 className="text-2xl md:text-3xl font-light mb-8 md:mb-10" style={{ color: "#3A0A14", fontFamily: "'Playfair Display', serif" }}>Royal Countdown</h2>
            
            <div className="flex gap-3 md:gap-6 justify-center">
              <FlipDigit value={time.days} label="Days" />
              <div className="flex flex-col justify-center text-xl md:text-2xl font-light pt-2" style={{ color: "#D4AF37" }}>:</div>
        ...
        <section className="py-20 md:py-24 px-4" style={{ background: "white" }}>
          <div className="max-w-3xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12 md:mb-16">
              <p className="font-sans text-[10px] md:text-xs uppercase tracking-[0.5em] mb-3" style={{ color: "#D4AF37" }}>Royal Programme</p>
              <h2 className="text-3xl md:text-4xl font-light" style={{ color: "#3A0A14" }}>The Events</h2>
              <MughalDivider />
            </motion.div>
            <div className="space-y-6">
              {events.map((ev, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                  ...
                  <p className="font-sans text-[10px] md:text-xs uppercase tracking-widest mb-2" style={{ color: "#D4AF37" }}>✦ Event {i + 1} ✦</p>
                  <h3 className="text-xl md:text-2xl font-light mb-2" style={{ color: "#3A0A14" }}>{ev.name}</h3>
                  {ev.description && <p className="font-sans text-xs md:text-sm mb-4 leading-relaxed" style={{ color: "#8A6A5A" }}>{ev.description}</p>}
                  <div className="flex flex-wrap gap-4 font-sans text-[10px] md:text-xs" style={{ color: "#B0906A" }}>
          ...
          <section className="py-20 md:py-24 px-4" style={{ background: "#FFF9F0" }}>
            <div className="max-w-5xl mx-auto">
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12 md:mb-14">
                <h2 className="text-3xl md:text-4xl font-light" style={{ color: "#3A0A14" }}>Royal Portraits</h2>
              </motion.div>
        ...
        {/* ── FOOTER ──────────────────────────────── */}
        <section className="py-16 md:py-20 text-center px-4" style={{ background: "linear-gradient(160deg, #1A0008, #3A0A14)" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <Crown className="w-8 h-8 mx-auto mb-6" style={{ color: "#D4AF37" }} />
            <h2 className="text-2xl md:text-3xl font-light mb-2" style={{ color: "#FFFBF0" }}>Your Grace Honours Us</h2>
            <p className="font-sans text-xs md:text-sm mb-10" style={{ color: "#806060" }}>{venue}</p>
