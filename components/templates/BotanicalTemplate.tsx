    ...
    <div className="min-h-screen" style={{ fontFamily: "'Playfair Display', Georgia, serif", background: "#FAFCF8" }}>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center text-center overflow-hidden px-6"
        ...
        <div className="absolute top-0 left-0 right-0 pointer-events-none">
          <svg width="100%" height="60" viewBox="0 0 1200 60" preserveAspectRatio="none">
            <motion.path d="M0 30 Q100 5, 200 30 Q300 55, 400 30 Q500 5, 600 30 Q700 55, 800 30 Q900 5, 1000 30 Q1100 55, 1200 30"
              stroke="#2D5A27" strokeWidth="1" fill="none" opacity="0.25"
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            className="font-sans text-xs uppercase tracking-[0.45em] mb-8" style={{ color: "#5A8A4A" }}>
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.5 }}
            className="font-sans text-base tracking-widest" style={{ color: "#5A8A4A" }}>{fmt}</motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 3 }}
            className="font-sans text-sm mt-2" style={{ color: "#8FAF7E" }}>{venue}</motion.p>
        </div>
      ...
      <section className="py-20 text-center" style={{ background: "white" }}>
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <p className="font-sans text-xs uppercase tracking-[0.4em] mb-10" style={{ color: "#5A8A4A" }}>Days Until We Say "I Do"</p>
          <div className="flex gap-6 md:gap-12 justify-center flex-wrap">
            {[{ label: "Days", v: time.days }, { label: "Hours", v: time.hours }, { label: "Minutes", v: time.minutes }, { label: "Seconds", v: time.seconds }].map(i => (
              <div key={i.label} className="flex flex-col items-center gap-2">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center text-2xl md:text-3xl font-light transition-all duration-500"
                  style={{ background: "linear-gradient(135deg, #F2F7EE, #E0EDD8)", color: "#1A3A14", boxShadow: "0 4px 20px rgba(45,90,39,0.12)" }}>
      ...
      <section className="py-24 px-4" style={{ background: "#F2F7EE" }}>
        <div className="max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <p className="font-sans text-xs uppercase tracking-[0.4em] mb-3" style={{ color: "#5A8A4A" }}>What's Planned</p>
            <h2 className="text-4xl font-light" style={{ color: "#1A3A14" }}>Garden Events</h2>
            <VineDivider />
          </motion.div>
            ...
            {events.map((ev, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h3 className="text-2xl font-light mb-2" style={{ color: "#1A3A14" }}>{ev.name}</h3>
                    {ev.description && <p className="font-sans text-sm mb-4" style={{ color: "#5A8A4A" }}>{ev.description}</p>}
        ...
        <section className="py-24 px-4" style={{ background: "white" }}>
          <div className="max-w-5xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
              <h2 className="text-4xl font-light" style={{ color: "#1A3A14" }}>Our Story in Photos</h2>
            </motion.div>
      ...
      <section className="py-20 text-center" style={{ background: "#F2F7EE" }}>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="text-3xl font-light mb-2" style={{ color: "#1A3A14" }}>Join Us in the Garden</h2>
