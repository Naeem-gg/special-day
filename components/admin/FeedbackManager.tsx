...
import { MessageSquare, AlertTriangle, Lightbulb, FileText, Send, CheckCircle2, Clock, Shield, ChevronDown, ChevronUp, Loader2, Search } from "lucide-react";
...
function FeedbackRow({ item, onUpdate }: { item: FeedbackItem; onUpdate: () => void }) {
  ...
  return (
    <div className="border border-gray-100 rounded-2xl overflow-hidden bg-white shadow-sm">
      ...
      >
        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold"
          style={{ background: typeInfo.bg, color: typeInfo.color }}>
          #{item.id}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-0.5">
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: typeInfo.color }}>{typeInfo.label}</span>
            <span className="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase" style={{ background: statusStyle.bg, color: statusStyle.color }}>
              {statusStyle.label}
            </span>
            {item.replies.some(r => r.isAdmin) && (
              <span className="text-[9px] font-bold text-[#F43F8F] bg-rose-50 px-2 py-0.5 rounded-full">Replied</span>
            )}
          </div>
          <p className="font-semibold text-gray-800 text-sm truncate">{item.subject}</p>
          <p className="text-[11px] text-gray-400">{item.name} · {item.email} · {new Date(item.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</p>
        </div>
        ...
        {open && (
          ...
          >
            <div className="px-4 pb-5 border-t border-gray-50 space-y-4">
              {/* Original message */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mt-4 mb-2">Original Message</p>
                <p className="text-sm text-gray-700 bg-gray-50 rounded-xl p-4 leading-relaxed">{item.message}</p>
              </div>

              {/* Status change */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Change Status</p>
              ...
              {item.replies.length > 0 && (
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Thread ({item.replies.length})</p>
                  <div className="space-y-2">
                    {[...item.replies].reverse().map(r => (
                      <div key={r.id} className={`flex gap-2 ${r.isAdmin ? "flex-row-reverse" : ""}`}>
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-[9px] font-bold ${r.isAdmin ? "bg-rose-100 text-[#F43F8F]" : "bg-gray-100 text-gray-500"}`}>
                          {r.isAdmin ? "DN" : r.authorName[0]}
                        </div>
                        <div className={`max-w-[85%] px-3 py-2 rounded-xl text-xs ${r.isAdmin ? "bg-gradient-to-br from-[#F43F8F] to-[#c73272] text-white rounded-tr-sm" : "bg-gray-100 text-gray-700 rounded-tl-sm"}`}>
                          {r.isAdmin && <p className="text-[8px] font-bold mb-0.5 text-white/70">ADMIN</p>}
                          {r.message}
                          <p className={`text-[9px] mt-0.5 ${r.isAdmin ? "text-white/50" : "text-gray-400"}`}>{new Date(r.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {/* Reply box */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">Reply Publicly as Admin</p>
...
export function FeedbackManager() {
  ...
  const filtered = items.filter(item => {
    ...
    const matchSearch = !search || item.subject.toLowerCase().includes(search.toLowerCase()) || item.name.toLowerCase().includes(search.toLowerCase());
    return matchStatus && matchSearch;
  });

  const counts = { all: items.length, open: items.filter(i => i.status === "open").length, in_progress: items.filter(i => i.status === "in_progress").length, resolved: items.filter(i => i.status === "resolved").length, closed: items.filter(i => i.status === "closed").length };
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={s.key} className="bg-gray-50 rounded-xl p-3 text-center">
            <p className="text-2xl font-bold" style={{ color: s.color }}>{counts[s.key as keyof typeof counts]}</p>
