...
export function TestimonialManager({ initialTestimonials }: { initialTestimonials?: any[] }) {
  ...
  const handleTogglePublic = async (id: number, currentStatus: boolean) => {
    try {
      ...
      if (res.ok) {
        ...
        // Optimistic update
        setTestimonials(prev => prev.map(t => t.id === id ? { ...t, isPublic: !currentStatus } : t));
      } else {
  ...
  if (loading && !initialTestimonials) return <div className="p-8 text-center">Loading testimonials...</div>;
            ...
            <Card key={t.id} className="overflow-hidden hover:shadow-md transition-shadow">
              ...
              <CardContent className="p-4 space-y-4">
                ...
                <div className="pt-2 flex items-center justify-between border-t border-gray-50">
                  ...
                  >
                    {t.isPublic ? <><Lock className="w-3 h-3 mr-1" /> Make Private</> : <><Globe className="w-3 h-3 mr-1" /> Publish Now</>}
                  </Button>
                </div>
              </CardContent>
            </Card>
