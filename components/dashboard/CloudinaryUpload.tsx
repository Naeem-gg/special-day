  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok && data.secure_url) {
        ...
        showMessage(
          'error',
          data.error?.message || 'Upload failed. Check your Cloudinary preset settings.'
        )
      }
    } catch (error) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center mb-1 px-1">
        <p className="text-sm font-medium text-gray-500">
          Uploaded: <span className="text-gray-900 font-semibold">{images.length}</span>{' '}
          <span className="text-gray-400">/ {maxUploads}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 md:grid-cols-4 gap-2 md:gap-4">
        {images.map((img) => (
          <div
            key={img.publicId}
            className="relative aspect-3/4 rounded-xl overflow-hidden border border-gray-200 shadow-xs group"
          >
            <Image
              src={img.url}
              alt="Gallery"
        ...
        {images.length < maxUploads && (
          <label
            className={`flex flex-col items-center justify-center aspect-3/4 border-2 border-dashed rounded-xl cursor-pointer transition-colors border-rose-200 hover:bg-rose-50 bg-rose-50/30 active:bg-rose-100`}
          >
          ...
          >
            <div className="flex flex-col w-full h-full md:max-w-md md:h-auto md:max-h-[90vh] bg-black md:bg-white md:rounded-4xl overflow-hidden shadow-2xl">
              ...
              <div className="flex-none p-4 pt-safe flex justify-between items-center bg-black/50 md:bg-gray-50/50 border-b border-white/10 md:border-gray-100 z-10">
                <h3 className="font-serif font-semibold text-lg text-white md:text-gray-800">
                  Adjust Photo
                </h3>
                <button
                  onClick={() => setImageSrc(null)}
                  className="p-2 bg-white/10 md:hover:bg-gray-200 rounded-full transition-colors text-white md:text-gray-500 active:scale-95"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex-none p-6 pb-safe bg-black md:bg-gray-50 flex flex-col gap-5 border-t border-white/10 md:border-gray-100 z-50 relative">
                <div className="flex items-center gap-4 px-2">
                  <span className="text-[10px] font-bold text-white/50 md:text-gray-400 uppercase tracking-wider">
                    Zoom
                  </span>
                  <input
                    type="range"
